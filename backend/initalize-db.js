...
const pickWallType = (tags) => {
    ...
    if (tags?.["indoor"] === "yes" || tags?.["indoor"] === "only")
        return "IndoorWall";
        ...
        try {
            ...
            const response = await axios.post(endpoint, payload, {
                headers: {
                    ...
                    "User-Agent":
                        "HookdImporter/1.0 (+https://github.com/mattia/Hookd)",
                },
                timeout: 120000,
            ...
            if (
                status &&
                ![406, 408, 429, 500, 502, 503, 504].includes(status)
            ) {
                throw error;
            }
        }
...
async function main() {
    ...
    const greatCellisWall = await Wall.findOneAndUpdate(
        { name: "Great Celli's Wall" },
        {
            $set: {
                name: "Great Celli's Wall",
                wallType: "IndoorWall",
                description:
                    "Iconic overhanging wall at Celli Walls, perfect for training and testing your skills.",
                location: {
                    type: "Point",
                    coordinates: [11.1217, 46.0667],
                    address: "Via Celli, 7, 38100 Trento",
                },
                difficulty: "ADVANCED",
                status: "OPEN",
                rating: 0,
    ...
    console.log(
        `Test wall "Great Celli's Wall" ready (id: ${greatCellisWall._id})`,
    );

    const celliWalls = await Facility.findOneAndUpdate(
        { username: "celli_walls" },
        {
            $set: {
                name: "Celli Walls",
                description:
                    "Renowned climbing gym in Trento, offering a variety of routes and training facilities.",
                location: {
                    type: "Point",
                    coordinates: [11.1217, 46.0667],
                    address: "Via Celli, 7, 38100 Trento",
                },
                walls: [greatCellisWall._id],
                password: seedPasswordHash,
                authMethods: ["local"],
            },
            $setOnInsert: {
                email: "celli.walls@hookd.internal",
                username: "celli_walls",
            },
        },
        { upsert: true, new: true },
    );
    console.log(`Facility "Celli Walls" ready (id: ${celliWalls._id})`);
    ...
    for (const el of elements) {
        ...
        if (wallType === "IndoorWall") {
            const facilitySlug = name
                ...
                .replace(/^_+|_+$/g, "")
                .slice(0, 30);
            const facilityUsername = `${facilitySlug || "facility"}_${Math.round(
                lat * 10000,
    ...
    console.log(
        `Outdoor walls linked to Regione Trentino: ${outdoorWallIds.length}`,
    );
}
